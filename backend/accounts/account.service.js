...
function randomTokenString() {
    ...
    const token = randomBytes.readUInt16BE(0); // Convert the bytes to a 16-bit unsigned integer
    const fourDigitToken = token % 10000; // Get the modulo of the token with 10000 to ensure it is within the range of four digits
    return fourDigitToken;
  }
/*
function randomTokenString() {
...
async function sendPasswordResetEmail(account, origin) {
    let message;
    if (origin) {
        const resetUrl = `${origin}/reset/${account.resetToken}`;
